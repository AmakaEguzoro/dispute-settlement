...
export class NqrserviceService {
  ...
  bulknqrOnboardUrl =
    "http://197.253.19.76:8019/api/v1/vas/nqr/onboard/create/bulk";
  merchantnqrOnboardUrl =
    "http://197.253.19.76:8019/api/v1/vas/nqr/ptsp/onboard";
  merchantnqrHistoryUrl = "http://197.253.19.76:8019/api/v1/vas/nqr/ptsp/list";
  transUrl = "http://197.253.19.76:8019/api/v1/vas/nqr/ptsp/transactions";

  test_key =
    "NGU1ODY3NTZjYmM3M2ViNGU5Nzk3ZjgzYTA4MWNiMWE0Y2JlNTg1OTY0MTFmM2Y1OTIyM2Q5NWRiNzQxNDNiMQ==";
  live_key =
    "ZmU1NzQ4MDdlNmM5YmU4ZDIyMWE1M2Y0MDc4MGU2MDg1N2I4Y2FjNmFkNTliNWUxMmI1OWQ1ZTJjZjAzMTA3MA==";
  ...
  bulkcreateSubMerchant(data) {
    ...
    return this.httpClient.post(this.bulknqrOnboardUrl, data, { headers }).pipe(
      map((response: any) => {
        const res = response;
        return res;
      })
  ...
  bulkcreateMerchant(data) {
    ...
    return this.httpClient
      .post(this.merchantnqrOnboardUrl, data, { headers })
      .pipe(
        map((response: any) => {
          const res = response;
          return res;
  ...
  exportHistory() {
    ...
    return this.httpClient.get(`${this.nqrHistoryUrl}?action=export`, {
      headers,
    });
  }

  getMerchantHistory(perPage = 50, page, query) {
    let headers = new HttpHeaders();
    function sha1Hmac(value, key) {
      return cryptojs.HmacSHA256(value, key);
    }
    ...
    let encrypt = btoa(
      encodeURI(
        `${this.merchantnqrHistoryUrl}?page=${page}&perPage=${perPage}&query=${query}`
      )
    );
    ...
    return this.httpClient.get(
      `${this.merchantnqrHistoryUrl}?perPage=${perPage}&page=${page}&query=${query}`,
      { headers }
    );
  }
  ...
  exportMerchantHistory() {
    ...
    return this.httpClient.get(`${this.merchantnqrHistoryUrl}?action=export`, {
      headers,
    });
  }

  getTransaction(page, perPage, startDate, endDate, query) {
    let headers = new HttpHeaders();
    function sha1Hmac(value, key) {
      return cryptojs.HmacSHA256(value, key);
    }
    ...
    let encrypt = btoa(
      encodeURI(
        `${this.transUrl}?daterange=${startDate} - ${endDate}&page=${page}&perPage=${perPage}&query=${query}`
      )
    );
