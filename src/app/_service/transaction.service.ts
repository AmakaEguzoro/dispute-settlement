...
export class TransactionService {
  ...
  pendingCreditUrl =
    "http://197.253.19.76:8018/api/v1/vas/vicebanking/withdrawal/post-pending-credit";
  ...
  getWalletBalance(walletBalance: WalletBalance) {
    return this.httpClient
      .post(`http://vas.itexapp.com/vas/wallet-balance`, walletBalance)
      .pipe(
        map((response: any) => {
  ...
  getTransactionLimits(transactionLimit: TransactionLimit) {
    return this.httpClient
      .post(
        `http://vas.itexapp.com/api/v1/wallet/reset-limit`,
        transactionLimit
      )
      .pipe(
        map((response: any) => {
  ...
  getAutopostTransactions(transaction: Transaction) {
    //   transaction / details / autopost;
    return this.httpClient
      .post(this.baseUrl + `details/autopost`, transaction)
      .pipe(
        map((response: any) => {
          const transactionSummary = response;
          return transactionSummary;
  ...
  getAutopostTransactionSummary(transaction: Transaction) {
    return this.httpClient
      .post(this.baseUrl + `details/summary`, transaction)
      .pipe(
        map((response: any) => {
