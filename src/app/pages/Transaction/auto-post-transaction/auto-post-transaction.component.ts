...
@Component({
  selector: "app-auto-post-transaction",
  templateUrl: "./auto-post-transaction.component.html",
  styleUrls: ["./auto-post-transaction.component.scss"],
})
export class AutoPostTransactionComponent implements OnInit {
  constructor(
  ...
  ngOnInit() {
    ...
    this.TransactionSummary(this.summaryload);
  }
  isData: boolean;
  loading = true;
  data: any;
  ...
  Transaction(payload) {
    this.isData = true;
    this.loading = true;
    this.transactionService.getAutopostTransactions(payload).subscribe(
      (data) => {
        this.loading = false;
  ...
  TransactionSummary(summaryload) {
    this.isData = true;
    this.isLoading = true;
    this.transactionService
      .getAutopostTransactionSummary(summaryload)
      .subscribe(
        (data) => {
  ...
  pageChanged(event: any): void {
    ...
    this.router.navigateByUrl("/transaction/autopost", {
  ...
  exportAsXLSX(): void {
    ...
    if (this.userName() != "Providus") {
      ...
      this.loading = true;
      this.transactionService
        .getAutopostTransactions(this.filterData)
        .subscribe(
          (data) => {
            this.loading = false;
